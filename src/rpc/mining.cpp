...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (!request.params[0].isNull())
    {
        ...
        if (strMode == "proposal")
        {
            ...
            CBlock block;
            if (!DecodeHexBlk(block, dataval.get_str(), true))
    ...
    const bool fPreSegWit = pindexPrev->nHeight < Params().GetConsensus().WitnessHeight;
    ...
    if (pindexPrev->nHeight >= Params().GetConsensus().CSVHeight)
        aRules.push_back("csv");
    if (pindexPrev->nHeight >= Params().GetConsensus().WitnessHeight)
...
static UniValue submitblock(const JSONRPCRequest& request)
{
    ...
    if (!DecodeHexBlk(block, request.params[0].get_str(), true)) {
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "Block decode failed");
    }
